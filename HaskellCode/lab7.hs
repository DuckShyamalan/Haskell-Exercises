data Expr = Var String | Lam String Expr | App Expr Expr | Cl Expr Env 
  deriving (Eq, Show, Read)

--type CEK = (Expr, Env, Kont)

type Env = [(String, Expr)]
--data Kont = "" | Ar(Expr, Env, Kont) | Fn(\x->Expr, Env, Kont)

lookup1 :: String -> Env -> Expr
lookup1 _ [] = error "Error: empty list"
--unbound error(above) ; e = Expr in the Env(below)
lookup1 x ((y,e):es) | x == y = e
                     | otherwise = lookup x es

type Frame = Expr
type Kont = Frame Kont
type Config = (Expr, Env, Kont)
